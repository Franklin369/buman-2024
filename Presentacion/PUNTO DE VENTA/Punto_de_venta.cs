...
namespace RestCsharp.Presentacion.PUNTO_DE_VENTA
{
    ...
    public partial class Punto_de_venta : UserControl
    {
        ...
        private void EventoDelegado()
        {
            ...
            btnvermesas.Click += new EventHandler((sender, args) =>
            {
                GuardarClick?.Invoke(this, null);
            });
        ...
        private void sumar1(ref DataTable dt)
        {
            ...
            if (Trabajasconimp == "SI")
            {
                ...
                impuestocalculado = Total - Total / 1.18m;
        ...
        private void btnEnviarpedido_Click(object sender, EventArgs e)
        {
            if (idventa > 0)
            {
                ...
                ImprimirPedido(idventa, rptPedidos);
        ...
        private void ImprimirPedido(int idventapasar, Telerik.ReportViewer.WinForms.ReportViewer reporteador)
        {
            ////var funcion = new Ddetalleventas();
            ////var parametros = new Ldetalleventas();
            ////var dt = new DataTable();
            ...
            ////************
            //string codigobar = "bar";
            //string codigococina = "pedidos";
            //var funcionBar = new Ddetalleventas();
            //var parametrosBar = new Ldetalleventas();
            //parametrosBar.idventa = idventa;
            //parametrosBar.codigoareaimpresion = codigobar;
            //var dtBar = new DataTable();
            //funcionBar.rptTicketCocinaXcodigoarea(ref dtBar, parametrosBar);
            //if (Tiponotas == "General")
            //{
            //    var rpt = new Rpedidos();
            //    rpt.DataSource = dtBar;
            //    rpt.Tabladetalle.DataSource = dtBar;
            ...
            //funcionImpresoras.ImprimirVarios(rptPedidos.ReportSource, codigobar);
            //// funcionImpresoras.ImprimirVariosPedidos(rptPedidos.ReportSource, parametrosImpresoras);


            ////foreach (DataRow data in dt.Rows)
            ////{
            ////    int Idareaimpresion = Convert.ToInt32(data["IdareaImpresion"]);
            ////    parametrosXareas.Idareaimpresion = Idareaimpresion;
            ////    parametrosXareas.idventa = idventa;
            ////    funcionXareas.rptTicketCocinaXidarea(ref dtXareas, parametros);
            ////    if (Tiponotas == "General")
            ////    {
            ////        var rpt = new Rpedidos();
            ////        rpt.DataSource = dtXareas;
            ////        rpt.Tabladetalle.DataSource = dtXareas;
            ////        rptPedidos.Report = rpt;
            ////    }
            ////    else
            ////    {
            ...
            ////    rptPedidos.RefreshReport();
            ////    var funcionImpresoras = new Dimpresoras();
            ////    var parametrosImpresoras = new Limpresoras();
            ////    parametrosImpresoras.Id_Areas_de_Impresion = Idareaimpresion;
            ...
            ImprimirarearBar(idventapasar, reporteador);
            Imprimirarearcocina(idventapasar, reporteador);

        }
        private void ImprimirarearBar(int idventapasar,Telerik.ReportViewer.WinForms.ReportViewer reporteador)
        {
            string codigobar = "bar";
            var funcion = new Ddetalleventas();
            var parametros = new Ldetalleventas();
            parametros.idventa = idventapasar;
            parametros.codigoareaimpresion = codigobar;
            var dt = new DataTable();
            funcion.rptTicketCocinaXcodigoarea(ref dt, parametros);
            if (dt.Rows.Count > 0)
            {
                if (Tiponotas == "General")
                {
                ...
                var funcionImpresoras = new Dimpresoras();
                funcionImpresoras.ImprimirVarios(reporteador.ReportSource, codigobar);
            }

        }
        private void Imprimirarearcocina(int idventapasar, Telerik.ReportViewer.WinForms.ReportViewer reporteador)
        {
            string codigococina = "pedidos";
            var funcion = new Ddetalleventas();
            var parametros = new Ldetalleventas();
            parametros.idventa = idventapasar;
            parametros.codigoareaimpresion = codigococina;
            var dt = new DataTable();
            funcion.rptTicketCocinaXcodigoarea(ref dt, parametros);
            if (dt.Rows.Count > 0)
            {
                ...
                reporteador.RefreshReport();
                var funcionImpresoras = new Dimpresoras();
                funcionImpresoras.ImprimirVarios(reporteador.ReportSource, codigococina);
            }
        }
        ...
        private void timerImprimir_Tick(object sender, EventArgs e)
        {
            ...
            if (idventaSolicitud > 0)
            {
                ...
                if (tipocaja == "PRINCIPAL")
                {
                    ...
                    ImprimirPedido(idventaSolicitud, rptComunicador);
                    EliminarSolImpri();
                    idventaSolicitud = 0;
                    timerImprimir.Start();
                }
