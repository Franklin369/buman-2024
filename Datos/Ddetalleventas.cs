...
namespace RestCsharp.Datos
{
    public  class Ddetalleventas
    {
        ...
        public void rptTicketCocinaXcodigoarea(ref DataTable dt, Ldetalleventas parametros)
        {
            try
            {
                CONEXIONMAESTRA.abrir();
                SqlDataAdapter da = new SqlDataAdapter("rptTicketCocinaXcodigoarea", CONEXIONMAESTRA.conectar);
                da.SelectCommand.CommandType = CommandType.StoredProcedure;
                da.SelectCommand.Parameters.AddWithValue("@Idventa", parametros.idventa);
                da.SelectCommand.Parameters.AddWithValue("@codigoarea", parametros.codigoareaimpresion);
