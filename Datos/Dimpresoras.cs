...
namespace RestCsharp.Datos
{
    public class Dimpresoras
    {
        ...
        public void ImprimirVariosPedidos(Telerik.Reporting.ReportSource reporte, Limpresoras parametrosPedir)
        {
            var funcion = new Dimpresoras();
            var dt = new DataTable();
            var parametros = new Limpresoras();
            parametros.Id_Areas_de_Impresion = parametrosPedir.Id_Areas_de_Impresion;
            funcion.mostrarImpresorasArea(ref dt, parametros);
            int contador = 0;
            contador = dt.Rows.Count;
            string Impresora;
            if (contador > 0)
